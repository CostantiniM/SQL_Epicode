...
CREATE TABLE store (CodiceStore INTEGER,
...
CREATE TABLE servizio_impiegato (Id_Servizio INTEGER,
CodiceFiscale VARCHAR(16),
CodiceStore INTEGER,
DataInizio DATE,
DataFine DATE,
Carica VARCHAR(30),
PRIMARY KEY AUTO_INCREMENT(Id_Servizio),
...
-- SELEZIONO TUTTI GLI IMPIEGATI CON LA LAUREA IN ECONOMIA
...
SELECT Nome, TitoloStudio
FROM impiegato
WHERE TitoloStudio IN ('Laurea in Economia'); 

-- SELEZIONO GLI IMPIEGATI CHE LAVORANO COME CASSIERE O CHE HANNO DIPLOMA DI INFORMATICA
...
SELECT i.Nome, si.Carica
FROM impiegato i
JOIN servizio_impiegato si
ON i.CodiceFiscale = si.CodiceFiscale
WHERE si.Carica IN ('Cassiere') OR  i.TitoloStudio IN ('Diploma di Informatica'); 
...
JOIN servizio_impiegato si
ON i.CodiceFiscale = si.CodiceFiscale
WHERE si.Carica = 'Cassiere' OR  i.TitoloStudio = 'Diploma di Informatica';

-- SELEZIONO I NOMI ED I TITOLI DEGLI IMPIEGATI CHE HANNO INIZIATO A LAVORARE DOPO IL 1 GENNAIO 2023

SELECT i.Nome, i.TitoloStudio
FROM impiegato i
JOIN servizio_impiegato si
ON i.CodiceFiscale = si.CodiceFiscale
WHERE si.DataInizio > '2023-01-01'; 

-- oppure la data senza ''
...
-- SELEZIONO I TITOLI DI STUDIO DISTINTI TRA GLI IMPIEGATI

SELECT DISTINCT TitoloStudio
FROM impiegato;

-- SELEZIONO GLI IMPIEGATI CON UN TITOLO DI STUDIO DIVERSO DA "LAUREA IN ECONOMIA"
...
SELECT Nome, TitoloStudio
FROM impiegato
WHERE TitoloStudio NOT IN ('Laurea in Economia'); ????

-- altro esempio:

SELECT Nome, TitoloStudio
FROM impiegato
WHERE TitoloStudio NOT IN ('Economia%');


-- SELEZIONO GLI IMPIEGATI CHE HANNO INIZIATO A LAVORARE PRIMA DEL 1 LUGLIO 2023 E SONO COMMESSI

SELECT i.Nome, si.DataInizio, si.Carica
FROM impiegato i
JOIN servizio_impiegato si
ON i.CodiceFiscale = si.CodiceFiscale
WHERE si.DataInizio <'2023-07-01' AND si.carica = 'Commesso';


-- SELEZIONO I TITOLI E GLI SVILUPPATORI DEI VIDEOGIOCHI DISTRIBUITI NEL 2020

SELECT Titolo, Sviluppatore, AnnoDistribuzione
FROM videogioco
WHERE AnnoDistribuzione >= '2020-01-01' AND AnnoDistribuzione <= '2020-12-01';
...
-- SELEZIONO I TITOLI DEI VIDEOGIOCHI DISPONIBILI NEI SETTORI 1 E 3 DEL NEGOZIO 5

-- SELEZIONO I NEGOZI CON PIU' DI 20 COPIE DISPONIBILI DI ALMENO UN VIDEOGIOCO
